...
.Launch the server running the script
[source, bash]
----
...
Unzip the RHDG server as in the previous example and duplicate the server folder to use one with each deployment. For example:
...
[source, bash]
----
./redhat-datagrid-8.1.0-server/bin/server.sh --server-root=redhat-datagrid-8.1.0-server/server1 --port-offset=0
./redhat-datagrid-8.1.0-server/bin/server.sh --server-root=redhat-datagrid-8.1.0-server/server2 --port-offset=100
----
