...
oc process -f templates/rhdg-01-operator.yaml -p OPERATOR_NAMESPACE=$RHDG_OPERATOR_NAMESPACE -p CLUSTER_NAMESPACE=$RHDG_NAMESPACE | oc apply -f -
...
oc process -f templates/rhdg-02-cluster.yaml -p CLUSTER_NAMESPACE=$RHDG_NAMESPACE -p CLUSTER_NAME=$RHDG_CLUSTER_NAME | oc apply -f -

# Create basic caches
echo -e "\n[3/8]Creating basic caches"
oc process -f templates/rhdg-03-caches.yaml -p CLUSTER_NAMESPACE=$RHDG_NAMESPACE -p CLUSTER_NAME=$RHDG_CLUSTER_NAME | oc apply -f -
...
oc process -f templates/rhdg-04-monitoring.yaml -p CLUSTER_NAMESPACE=$RHDG_NAMESPACE -p CLUSTER_NAME=$RHDG_CLUSTER_NAME | oc apply -f -
...
oc process -f templates/grafana-01-operator.yaml -p OPERATOR_NAMESPACE=$GRAFANA_NAMESPACE | oc apply -f -
...
oc process -f templates/grafana-02-instance.yaml -p OPERATOR_NAMESPACE=$GRAFANA_NAMESPACE | oc apply -f -
...
oc process -f templates/grafana-03-datasource.yaml -p BEARER_TOKEN=$BEARER_TOKEN -p OPERATOR_NAMESPACE=$GRAFANA_NAMESPACE | oc apply -f -
...
oc process -f templates/grafana-04-dashboard.yaml -p DASHBOARD_NAME=$GRAFANA_DASHBOARD -p OPERATOR_NAMESPACE=$GRAFANA_NAMESPACE | oc apply -f -
